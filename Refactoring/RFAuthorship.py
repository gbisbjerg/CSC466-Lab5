...
import json
import pandas as pd
from utils import mkdir_p, getGroundTruth
...
def RandomForest(authorName, NumAttributes, NumDataPoints, NumTrees, save_trees_flag = "F", threshold = 0.2, restrictionsLoc = None):
    ...
    transform_df_to_forest_input("TF_IDF.csv", directory, authorName, "RF/TF_IDF{}.csv".format(authorName))
    df_file_csv = "RF/TF_IDF{}.csv".format(authorName)
    D, A, initial_T, C = rf_format_data(df_file_csv, threshold, restrictionsLoc)

    with open(df_file_csv) as f:
        documents = [row.split(',')[0] for row in f]
    ...
    mkdir_p("RF_predictions")
    res_DF.to_csv("./RF_predictions/" + authorName + ".csv", index=True)
